...
function renderSymbols() {
    ...
    if (!isEditor) {
        items = items.filter(item => item.is_visible);
    }
...
function createSymbolCell(symbol) {
    ...
    if (currentMode === 'editor' && !symbol.is_visible) {
...
async function confirmEditItem(itemType) {
    ...
    if (isEditing) {
        ...
        const newItem = { ...data, id: generateUniqueId(), is_visible: true };
        if (itemType === 'category') {
...
function showContextMenu(e, id) {
    ...
    if(found.item.is_hideable === false){
        dom.contextMenuToggleVisibility.style.display = 'none';
    }
...
async function toggleItemVisibility(itemId) {
    ...
    if (!found || found.item.is_hideable === false) return;

    const { item } = found;
    // Ensure is_visible property exists, default to true if not
    if (typeof item.is_visible !== 'boolean') {
        item.is_visible = true;
    }
    
    item.is_visible = !item.is_visible; // Toggle visibility
    ...
    try {
        ...
        console.log(`Item ${itemId} visibility set to ${item.is_visible}`);
    } catch (error) {
        console.error('Error updating item visibility:', error);
        alert('Could not save visibility change. Reverting.');
        item.is_visible = !item.is_visible; // Revert on error
        renderSymbols();
    }
}
